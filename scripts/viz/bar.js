...
function BarViz (logs) {
  ...
  function parse (logs, parts = 51) {
    ...
    for (const id in logs) {
      ...
      if (!h[Math.floor(pos)]) { h[Math.floor(pos)] = { audio: 0, visual: 0, research: 0, physical: 0 } }
      if (!h[Math.ceil(pos)]) { h[Math.ceil(pos)] = { audio: 0, visual: 0, research: 0, physical: 0 } }
  ...
  this.draw = function () {
    ...
    return Object.keys(segments).reduce((acc, val, id) => {
      ...
      const physical_h = clamp(seg.physical * mod, 4, 100)
      const physical_y = (physical_h + research_y) + 0.5
      return `${acc}
      ...
      <rect class='physical' x='${x}' y='${125 - physical_y}' width='${cell}' height='${physical_h}' rx="2" ry="2"></rect>`
    }, '')
  }
